...
#ACS Name:
#Main Character Name: 
#Extracted rig from matching .rtr.adbi file for Main Character:
#Character Variation Count: 
#Variations: 
#Animation Count: 1
#Animationlist
#Matched Animation MB files
#Msssing Animation MB files
#
...
def match_model_references(anim_sets_info, mb_files):
    for anim_set in anim_sets_info:
        ...
        for model_ref in anim_set.get('model_references', []):
            print(f"Checking model reference: {model_ref}")
            found_match = False
            for key, file_path in mb_files.items():
                base_filename = os.path.splitext(os.path.basename(key))[0]
                #print(f"Comparing with key: {key}, base_filename: {base_filename}")
                if model_ref[1] == base_filename:  # Compare only the second element of model_ref tuple
                    print(f"Match found: {file_path}")
                    model_ref_paths.append(file_path)
                    found_match = True
                    break
            if not found_match:
                print(f"No match found for {model_ref[1]}")  # Print only the second element of model_ref tuple
...
def calculate_totals(anim_sets_info, animation_counts):
    ...
    # Calculate duplicate_animation_count for each anim_set_info
...
def export_to_csv(anim_sets_by_folder, animation_counts, totals, subfolder, filename_prefix='anim_sets_info'):
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        # Write total headers and values
        ...
        # Write animation set headers with the 'Folder' column
        writer.writerow(['Source Folder'] + anim_set_headers)

        # Sort the folders and write the animation set data
...
def main(root_directory):
    ...
    animation_counts = {}  # Initialize animation_counts dictionary
        ...
        for filename in filenames:
            if filename.endswith('.anim_set'):
                ...
                if anim_set_info:
                    ...
                    # Determine the folder name based on the immediate subdirectories
